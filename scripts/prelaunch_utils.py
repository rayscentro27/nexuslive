...
def build_tester_email(name: str, login_link: str, membership_level: str, note: str = "") -> dict[str, str]:
    ...
    body_lines = [
        f"Hi {friendly_name},",
        "",
        "Welcome to the Nexus tester program.",
        "",
        f"Login here: {login_link}",
        "",
        "What to test:",
        "1. onboarding",
        "2. credit upload/analysis",
        "3. business setup checklist",
        "4. funding roadmap",
        "5. messaging/admin communication",
        "",
        "Please report any bugs, confusing steps, or rough edges you hit.",
        f"Your payment has been waived for test access on the {membership} level.",
    ]
    if note:
        body_lines.extend(["", f"Admin note: {note}"])
    body_lines.extend(["", "Thank you for helping us tighten the Nexus launch."])
    return {
        "subject": "Welcome to the Nexus tester program",
        "body": "\n".join(body_lines),
    }
